  ...
  <ConsolePageLayout>
    ...
    >
      <DmToolbar>
        ...
        <div slot="right">
          <el-select v-model="bindParams.is_delete" clearable placeholder="删除状态" style="width:120px;margin-right: 10px;" @change="$refs.DmData.initPage()">
            <el-option :value="1" label="是"/>
            <el-option :value="0" label="否"/>
          </el-select>
          <el-select v-model="bindParams.status" clearable placeholder="启用状态" style="width:120px;margin-right: 10px;" @change="$refs.DmData.initPage()">
            <el-option :value="1" label="启用"/>
            <el-option :value="0" label="禁用"/>
          </el-select>
          <InputSearch v-model="bindParams.ip" placeholder="ip" @submit="$refs.DmData.initPage()" style="width:180px;"/>
        </div>
      </DmToolbar>
      <DmTable
        :loading="loading"
